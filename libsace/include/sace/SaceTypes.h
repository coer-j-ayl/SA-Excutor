...
#define LABEL_TO_SEQUENCE(x)     static_cast<uint32_t>(static_cast<uint64_t>(x) & 0xFFFFFFFF)
#define SEQUENCE_TO_LABEL(x, y)  ((static_cast<uint64_t>(x) & 0xFFFFFFFF) | (static_cast<uint64_t>(y) << 32))

/* Note : we consider ENUM as uint8_t when parcelable
 */
namespace android {

enum SaceCommandType: int8_t {
    SACE_TYPE_NORMAL,
    SACE_TYPE_SERVICE,
    SACE_TYPE_EVENT,
};
...
enum SaceServiceCommandType: int8_t {
...
enum SaceNormalCommandType: int8_t {
    SACE_NORMAL_CMD_START,
    SACE_NORMAL_CMD_CLOSE,
    SACE_NORMAL_CMD_DESTROY,
};

enum SaceEventType: int8_t {
    SACE_EVENT_TYPE_ADD,
    SACE_EVENT_TYPE_DEL,
    SACE_EVENT_TYPE_INFO,
};

enum SaceServiceFlags: int8_t {
    SACE_SERVICE_FLAG_NORMAL,
    SACE_SERVICE_FLAG_EVENT,
};

enum SaceCommandFlags: int8_t {
    SACE_CMD_FLAG_IN,
    SACE_CMD_FLAG_OUT,
};

enum SaceEventFlags: int8_t {
...
// ----------------------------------------------------
enum SaceResultHeaderType: int8_t {
    SACE_BASE_RESULT_TYPE_NORMAL,
    SACE_BASE_RESULT_TYPE_RESPONSE,
};

class SaceResultHeader : public Parcelable, public RefBase {
public:
    uint32_t len;
    ...
    SaceResultHeader& operator= (const SaceResultHeader &header) {
        len  = header.len;
        type = header.type;
        label = header.label;
...
enum SaceResultType {
    ...
    SACE_RESULT_TYPE_START,
    SACE_RESULT_TYPE_CLOSE,
    SACE_RESULT_TYPE_FD,
...
public:
    uint32_t sequence;   // Corresponding to command/service
    ...
    virtual void operator= (const SaceResult &reslt);
...
public:
    ...
    virtual void operator= (const SaceStatusResponse &response);
