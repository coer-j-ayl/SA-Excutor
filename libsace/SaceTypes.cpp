...
uint32_t SaceResultHeader::parcelSize () {
    return sizeof(uint32_t) + sizeof(int8_t) + sizeof(uint64_t);
}
...
// SaceResult ------------------------------------------------
void SaceResult::operator= (const SaceResult &rslt) {
    SaceResultHeader::operator=(rslt);
...
string SaceResult::mapResultTypeStr (enum SaceResultType type) {
    switch (type) {
        ...
        case SACE_RESULT_TYPE_START:
            return "SACE_RESULT_TYPE_START";
        case SACE_RESULT_TYPE_CLOSE:
            return "SACE_RESULT_TYPE_CLOSE";
        default:
            return "UNKNOWN";
    }
}
...
// SaceResponse ---------------------------------------------------------
void SaceStatusResponse::operator= (const SaceStatusResponse &response) {
    SaceResultHeader::operator=(response);
    name     = response.name;
    type     = response.type;
