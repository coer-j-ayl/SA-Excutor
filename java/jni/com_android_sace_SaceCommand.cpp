...
JNIEXPORT jint JNICALL Java_com_android_sace_SaceCommand_nGetError (JNIEnv *env __unused, jobject obj __unused,
    jlong ptr) {
    sp<SaceCommandObj> cmd = reinterpret_cast<SaceCommandWrapper*>(ptr)->cmd;
    return static_cast<jint>(cmd->getError());
}

JNIEXPORT jstring JNICALL Java_com_android_sace_SaceCommand_nGetCmd (JNIEnv *env __unused, jobject obj __unused,
    jlong ptr) {
    sp<SaceCommandObj> cmd = reinterpret_cast<SaceCommandWrapper*>(ptr)->cmd;
    return env->NewStringUTF(cmd->getCmd().c_str());
}
